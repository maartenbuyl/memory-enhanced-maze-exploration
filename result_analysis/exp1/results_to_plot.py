...
# Success rates
for i in range(2):
    ax[0, i].set(xlabel="Epochs", ylabel="success rate", ylim=(-0.05, 1.05))

    for j in range(len(datas)):
        epochs = np.arange(1, actual_nb_epochs)
        ax[0, i].plot(epochs, datas[j][i*2], label=labels[j])
    ax[0, i].legend(frameon=False, framealpha=1)


ax[0, 0].set_title('Training Set', fontweight='bold', size=16, pad=15)
ax[0, 1].set_title('Test Set', fontweight='bold', size=16, pad=15)

# Fail rates
for i in range(2):
    ax[1, i].set(xlabel="Epochs", ylabel="fail rate", ylim=(-0.05, 1.05))

    for j in range(len(datas)):
        epochs = np.arange(1, actual_nb_epochs)
        ax[1, i].plot(epochs, datas[j][1+i*2], label=labels[j])
    ax[1, i].legend(frameon=False, framealpha=0.5)
