...
class ACMemGRU(ACMem):
    # Overwrites
    output_size = ACMem.output_size + ACMem.memory_feature_size  # Add write output

    def __init__(self, config):
        self.h = None

        super(ACMemGRU, self).__init__(config)
    ...
    def forward(self, observation):
        ...
        output = torch.cat((context, state, local_memory, write_update), dim=1)
        return output

    def new_episode(self, starting_infos):
        super(ACMemGRU, self).new_episode(starting_infos)
    ...
    def update_task_info(self, non_terminated_indices, infos):
        super(ACMemGRU, self).update_task_info(non_terminated_indices, infos)
